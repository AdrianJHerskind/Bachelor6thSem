	...
	for(int j = 0; j < perpendicular.size(); j++)
	{
		...
		if(hit)
		{
			...
			if(firstPoint)
			{
				if(j-60 > 0)
				{
					cannyImage.at<Vec3b>(perpendicular.at(j-60)) = color0;
					firstPointPoint = perpendicular.at(j-60);
					firstPoint = false;
					secondPoint = true;
				}
			}
			...
			{
				secondPoint = false;
				if(j+67 < perpendicular.size())
				{
					cannyImage.at<Vec3b>(perpendicular.at(j+67)) = color0;
					secondPointPoint = perpendicular.at(j+67);
				}
			}
		}
	}
...
std::vector<Point> getStitchMiddlePoints(std::vector<Point> line)
{
	...
	{	
		...
		if(line.size()-i < stitchDistanceInPixel && line.size()-i > stitchDistanceInPixel/2)
		{
			stitchMiddlePoints.push_back(Point(line.at(line.size()-1)));
		}
		std::cout << "line at " << i << " is " << line.at(i) << "\n";
...
std::vector<Point> getExtremesLine(Mat cannyImage)
{
	...
	{
		std::cout << "ExtremesLine Length " << linePositions.size() << "\n";
...
Mat getCannyImage(Mat colorThresholdImage)
{
    ...
    Mat imageDilate(0,0,CV_8U);
    Mat imageErode(0,0,CV_8U);
    /*Mat DilateProject(0,0,CV_8UC3);
    cameraImage.copyTo(DilateProject);*/
	int edgeThresh = 1;
	int lowThreshold = 100;
	int const max_lowThreshold = 100;
	int ratio = 4;
	int kernel_size = 3;
	dilate(cannyInput, imageDilate, Mat(Size(9,9), CV_8U), Point(-1,-1));
	imshow("viewDilate", imageDilate);
   	 erode(imageDilate, imageErode, Mat(Size(9,9), CV_8U), Point(-1,-1));
	/*int pixelCounter = 0;
	for(int i = 0; i < imageErode.cols; i++)
	{
		for(int j = 0; j < imageErode.rows; j++)
		{
			Vec3b color = imageErode.at<Vec3b>(Point(i,j));
			if(color[0] == 255 && color[1] == 255 && color[2] == 255)
			{
				DilateProject.at<Vec3b>(Point(i,j)) = color;
				pixelCounter++;
			}
		}
	}
	std::cout << pixelCounter << "\n";
	imwrite("Image.jpg", DilateProject);*/
	imshow("viewErode", imageErode);
	//imshow("viewErode", imageErode);
	Canny( imageErode, imageWithCanny, lowThreshold, lowThreshold*ratio, kernel_size );
	Mat dst;	
...
Mat getThresholdImage(Mat averagedImage)
{
	...
	for(int i = 0; i < averagedImage.cols; i++)
	{
		for(int j = 0; j < averagedImage.rows; j++)
		{
			...
			if(abs(color[1] - color[2]) < 30 && color[1] > 51 && color[2] > 51 && color[0] < 100 && abs(color[1] - color[0]) > 50)
			{
			    	color[0] = 255;
				color[1] = 255;
				color[2] = 255;
...
Mat getAverageImage(int numberOfImagesRead)
{
	...
	if(numberOfImagesRead == 100)
	{
		...
		for(int i = 0; i < cameraImage.cols; i++)
		{
			...
			{
				Vec3b color;
				color[0] = (int)(averageArray[i][j][0] / 100.0);
				color[1] = (int)(averageArray[i][j][1] / 100.0);
				color[2] = (int)(averageArray[i][j][2] / 100.0);
...
int main(int argc, char **argv)
{
	...
	ros::init(argc, argv, "davinci_imageProcessing_node");
	ros::NodeHandle nh;
	...
	namedWindow("viewDilate", WINDOW_NORMAL);
	namedWindow("viewErode", WINDOW_NORMAL);
	startWindowThread();
			...
			if(continueToNextPoint)
			{
				...
				else
				{
					...
					if(!middleStitchPoints.empty())
					{
						...
						std::cout << "Next Middle Stitch Point X " << nextMiddleStitchPoint.x << "\n";
						std::cout << "Next Middle Stitch Point Y " << nextMiddleStitchPoint.y << "\n";
	...
	cv::destroyWindow("viewDilate");
	cv::destroyWindow("viewErode");
}
